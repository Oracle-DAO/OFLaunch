...
contract FairLaunchOF {
    ...
    function participate(uint256 amount_) external {
        ...
        require(userToAmount[msg.sender].add(amount_) <= maxPrincipalAllocation(), "Principal Amount exceeded for User");
        if(userToAmount[msg.sender] == 0){
            totalParticipatedUser += 1;
        }

        totalAmountRaised += amount_;
        userToAmount[msg.sender] = userToAmount[msg.sender].add(amount_);
        principalToken.safeTransferFrom(msg.sender, address(this), amount_);
    }

    function maxPrincipalAllocation() public view returns(uint256 _token) {
        uint256 timeBlocks = (uint32(block.timestamp).sub32(startTimestamp)).div(timeFrame);
        _token = ((timeBlocks > 20 ? 15000 : 7500 + (625 * timeBlocks)))*getPrincipalTokenDecimals();
    }

    function maxPrincipalForUser(address _userAddress) public view returns(uint256) {
        require(_userAddress != address(0), 'Invalid Address');
        return maxPrincipalAllocation().sub(userToAmount[_userAddress]);
    }

    function getPrincipalTokenDecimals() internal view returns(uint256) {
        return IERC20(principalToken).decimals();
    }
    ...
    function getPrice() external view returns(uint256 _price) {
        _price = totalAmountRaised > totalAmountToRaise ? getPriceForSC() : price;
    }

    function getPriceForSC() internal view returns(uint256) {
        return ((FixedPoint.fraction(totalAmountRaised, totalTokenSupply).decode112with18()).div(1e15)).mul(1e15);
    }
